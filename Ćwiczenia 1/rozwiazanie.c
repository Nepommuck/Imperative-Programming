...
int main(void) {
	...
	switch (to_do) {
		case 1: // linspace
		    if (TEST) printf("LINSPACE: Wpisz liczby: start, stop, n ");
            scanf("%lf %lf %d", &start, &stop, &len);
		...
		case 4: // multiply by scalar
			...
			break;
		case 5: // range
		    if (TEST) printf("RANGE: Wpisz liczby: start, step, n: ");
            scanf("%lf %lf %d", &start, &step, &len);
	        if(len < 0 || len > N) { printf("n<0 lub n > 100\n");  break; }
